...
//
function format(d){return d.toString()}!function(){$("body").on("mouseenter",'[data-toggle="tooltip"]',function(event){$(this).tooltip("show")}).on("mouseleave",'[data-toggle="tooltip"]',function(event){$(this).tooltip("hide")});var badBlocks=$("#reportsTable").dataTable({bProcessing:!0,bServerSide:!0,sAjaxSource:"/mongo/get/datatable",columns:[{"class":"details-control",data:"block"},{data:"errorType"},{data:"category"},{data:"ip"},{data:"createdAt"}],order:[[4,"desc"]]}),detailRows=[];$("#example tbody").on("click","tr td.details-control",function(){var tr=$(this).closest("tr"),row=dt.row(tr),idx=$.inArray(tr.attr("id"),detailRows);row.child.isShown()?(tr.removeClass("details"),row.child.hide(),detailRows.splice(idx,1)):(tr.addClass("details"),row.child(format(row.data())).show(),-1===idx&&detailRows.push(tr.attr("id")))}),badBlocks.on("draw",function(){$.each(detailRows,function(i,id){$("#"+id+" td.details-control").trigger("click")})})}();
//
